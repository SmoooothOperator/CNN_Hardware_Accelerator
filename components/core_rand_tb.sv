...
initial begin
  main();
  $display("Random stimulus generation completed.");
end
...
initial begin 
  ...
  execute  = 0;
  load     = 0;
  mode_sel = 0; // output stationary
  os_out   = 0;
  two_bit_mode = 0;
  load_b2 = 0;

  $dumpfile("core_rand_tb.vcd");
  $dumpvars(0,core_rand_tb);

  x_file = $fopen("rand_stim/activation_tile0.txt", "r");
  ...
  for (kij=0; kij<9; kij=kij+1) begin  // kij loop
    ...
    case(kij)
     0: w_file_name = "rand_stim/weight_itile0_otile0_kij0.txt";
     1: w_file_name = "rand_stim/weight_itile0_otile0_kij1.txt";
     2: w_file_name = "rand_stim/weight_itile0_otile0_kij2.txt";
     3: w_file_name = "rand_stim/weight_itile0_otile0_kij3.txt";
     4: w_file_name = "rand_stim/weight_itile0_otile0_kij4.txt";
     5: w_file_name = "rand_stim/weight_itile0_otile0_kij5.txt";
     6: w_file_name = "rand_stim/weight_itile0_otile0_kij6.txt";
     7: w_file_name = "rand_stim/weight_itile0_otile0_kij7.txt";
     8: w_file_name = "rand_stim/weight_itile0_otile0_kij8.txt";
    endcase
    ...
    // $display("// -----------------------------------------------------------------------");
    // $display("\nKernel weights (kij) loaded into XMEM (Addresses 1024 to %0d):", 1024+col-1);
    // $display("Addr | Binary                           | Hex");
    // $display("-----|----------------------------------|---------");
    
    // for (i = 1024; i < (col + 1024); i = i + 1) begin
    //     // NOTE: 'mem' must match the variable name of the reg array inside sram_32b_w2048.v
    //     // If it is named 'memory' or 'ram', change .mem[i] to .memory[i]
    //     $display("%4d | %b | %h \n", i, core_instance.xmem_inst.memory[i], core_instance.xmem_inst.memory[i]);
    // end
    ...
    $display("R%0d | %h %h | %h %h | %h %h | %h %h | %h %h | %h %h | %h %h | %h %h |", R, \
       `TILE(R,1).b_q, `TILE(R,1).a_q, \
       `TILE(R,2).b_q, `TILE(R,2).a_q, \
       `TILE(R,3).b_q, `TILE(R,3).a_q, \
       `TILE(R,4).b_q, `TILE(R,4).a_q, \
       `TILE(R,5).b_q, `TILE(R,5).a_q, \
       `TILE(R,6).b_q, `TILE(R,6).a_q, \
       `TILE(R,7).b_q, `TILE(R,7).a_q, \
       `TILE(R,8).b_q, `TILE(R,8).a_q);
  ...
  // with no parallelism 
    ...
    // $display(" %2d |  0  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q8); \
    // $display(" %2d |  1  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q9); \
    // $display(" %2d |  2  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q10); \
    // $display(" %2d |  3  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q11); \
    // $display(" %2d |  4  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q12); \
    // $display(" %2d |  5  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q13); \
    // $display(" %2d |  6  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q14); \
    // $display(" %2d |  7  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q15); \
    // $display(" %2d |  8  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q16); \
    // $display(" %2d |  9  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q17); \
    // $display(" %2d | 10  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q18); \
    // $display(" %2d | 11  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q19); \
    // $display(" %2d | 12  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q20); \
    // $display(" %2d | 13  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q21); \
    // $display(" %2d | 14  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q22); \
    // $display(" %2d | 15  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q23); \
    // $display(" %2d | 16  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q24); \
    // $display(" %2d | 17  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q25); \
    // $display(" %2d | 18  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q26); \
    // $display(" %2d | 19  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q27); \
    // $display(" %2d | 20  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q28); \
    // $display(" %2d | 21  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q29); \
    // $display(" %2d | 22  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q30); \
    // $display(" %2d | 23  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q31); \
    // $display(" %2d | 24  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q32); \
    // $display(" %2d | 25  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q33); \
    // $display(" %2d | 26  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q34); \
    // $display(" %2d | 27  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q35); \
    // $display(" %2d | 28  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q36); \
    // $display(" %2d | 29  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q37); \
    // $display(" %2d | 30  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q38); \
    // $display(" %2d | 31  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q39); \
    // $display(" %2d | 32  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q40); \
    // $display(" %2d | 33  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q41); \
    // $display(" %2d | 34  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q42); \
    // $display(" %2d | 35  | %h", ROW_IDX, core_instance.corelet_instance.l0_instance.row_num[ROW_IDX].fifo_instance.q43); \
    // $display("----|-------|-------------");
    ...
    for (t=0; t<len_nij+row*2; t=t+1) begin  // col+row*2 because that is the total cycle to populate the array
      #0.5 clk = 1'b0;   
      // Only need to send inst_w once, as it gets passed
  ...
  ////////// Accumulation /////////
  out_file = $fopen("rand_stim/out.txt", "r");  
  acc_file = $fopen("rand_stim/acc_file.txt", "r");  
