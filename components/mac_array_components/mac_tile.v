...
assign out_s = os_out ? (two_bit_mode ? c_q2 : c_q): mode_sel ? b_q : two_bit_mode ? out_s_2bitmode : out_s_4bitmode;
...
always@ (posedge clk) begin
        if (reset == 1) begin
                ...
                // this could be a bad design choice, but needed for now for SIMD to work on weight stationart, havent investigated why
                if(mode_sel)begin
                        b_q <= 0;
                        b2_q <= 0;
                end
        ...
        if(~mode_sel | os_out)begin // if os out is high it means output stationary calculation is done, ready to pass out data
...
mac2 #(.bw(bw), .psum_bw(psum_bw)) mac2_instance_hi (
        .a(ahi_q), 
        .b(b2_q),
        .c(c_q2), // changed this to c_q2 for output stationary. c_q2 should be 16'b0 anyways when output stationary not activated
        .out(out_s_hi)
);
