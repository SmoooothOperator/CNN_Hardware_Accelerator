...
initial begin 
  ...
  $dumpfile("core_os_rand_tb.vcd");
  $dumpvars(0,core_os_rand_tb);
  ...
  #0.5 clk = 1'b1;   
  /////////////////////////
  for (ic=0; ic<8; ic=ic+1) begin  // input channel loop (ic < 8)
    ...
    for (t=0; t<len_nij; t=t+1) begin  //iterate nij times to get all the rows
      ...
      // 2. OUTPUT STATIONARY ADAPTATION:
      // Take the 4 bits corresponding to Row ic (LSBs [ic*4 + :4]) and store into out temp storage.
      case(ic)
        0: act_cache[t*4 +: 4] = D_xmem[ 3: 0];
        1: act_cache[t*4 +: 4] = D_xmem[ 7: 4];
        2: act_cache[t*4 +: 4] = D_xmem[11: 8];
        3: act_cache[t*4 +: 4] = D_xmem[15:12];
        4: act_cache[t*4 +: 4] = D_xmem[19:16];
        5: act_cache[t*4 +: 4] = D_xmem[23:20];
        6: act_cache[t*4 +: 4] = D_xmem[27:24];
        7: act_cache[t*4 +: 4] = D_xmem[31:28];
      endcase
    end
    ...
    for (t=0; t<len_kij; t=t+1) begin
      ...
      // Input Width is 6.
                        ...
                        // Formula: (t/3)*6 + (t%3)
      D_xmem = { 
        // --- Output Row 1 (Pixels 1,0 to 1,3) mapped to Rows 4-7 ---
        // Row 7 (Out 1,3) uses Input [Base + 6 + 3] = [Base + 9]
        act_cache[((t/3)*6 + (t%3) + 9)*4 +: 4],
        // Row 6 (Out 1,2) uses Input [Base + 6 + 2] = [Base + 8]
        act_cache[((t/3)*6 + (t%3) + 8)*4 +: 4],
        // Row 5 (Out 1,1) uses Input [Base + 6 + 1] = [Base + 7]
        act_cache[((t/3)*6 + (t%3) + 7)*4 +: 4],
        // Row 4 (Out 1,0) uses Input [Base + 6 + 0] = [Base + 6]
        act_cache[((t/3)*6 + (t%3) + 6)*4 +: 4],

        // --- Output Row 0 (Pixels 0,0 to 0,3) mapped to Rows 0-3 ---
        // Row 3 (Out 0,3) uses Input [Base + 3]
        act_cache[((t/3)*6 + (t%3) + 3)*4 +: 4],
        // Row 2 (Out 0,2) uses Input [Base + 2]
        act_cache[((t/3)*6 + (t%3) + 2)*4 +: 4],
        // Row 1 (Out 0,1) uses Input [Base + 1]
        act_cache[((t/3)*6 + (t%3) + 1)*4 +: 4],
        // Row 0 (Out 0,0) uses Input [Base + 0]
        act_cache[((t/3)*6 + (t%3) + 0)*4 +: 4]
    ...
    for (t=0; t<8 + 2; t=t+1) begin  // set to 8 since we are only mapping 8 output_nij. +3 for the delay from testbench to ofifo valid to array
      #0.5 clk = 1'b0;  
    ...
    for (t=0; t<8; t=t+1) begin  // set to 8 since we are only mapping 8 output_nij
      #0.5 clk = 1'b0;  
      // ofifo read enable
      ofifo_rd = 1;
  ...
  // The first row is at the last address, which is 8, since 
  // we only mapped 8 o_nij
  A_pmem = 7;
  #0.5 clk = 1'b0; 
  #0.5 clk = 1'b1; 

  for (i=0; i<8+1; i=i+1) begin // len_onij is 8 since we only mapped 8 for o_stationary
